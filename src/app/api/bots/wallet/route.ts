...
export async function POST(req: NextRequest) {
  ...
  if (body.kind === "withdraw") {
    return NextResponse.json({ error: "السحب النقدي غير متاح. النقاط تُستخدم داخل البوت فقط." }, { status: 400 });
  }
  ...
  if (!publicCode || !uid || !amount || !reference) {
    return NextResponse.json({ error: "أكمل الرصيد ومرجع التحويل ومعرف العضو" }, { status: 400 });
  }
