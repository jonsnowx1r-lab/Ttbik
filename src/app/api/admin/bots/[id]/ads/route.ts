...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const channelUsername = String(body.channelUsername || "").trim().replace(/^@/, "") || null;
  if (!title) return NextResponse.json({ error: "أدخل عنوان الإعلان" }, { status: 400 });

  const { data, error } = await supabaseAdmin()
    .from("bot_ads")
    .insert({ bot_id: params.id, title, reward_points: rewardPoints, is_active: true, channel_username: channelUsername })
    .select("id, title, reward_points, is_active, channel_username")
    .single();
