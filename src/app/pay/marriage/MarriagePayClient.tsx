...
// MARRIAGE_BOT's own deposit page — a separate component from
// src/app/pay/PayClient.tsx (AD_BOT's) so it always posts to
// /api/payments/marriage-create-invoice, never the AD_BOT route.
export default function MarriagePayClient({ uid, justPaid }: { uid: string; justPaid: boolean }) {
  ...
  async function pay() {
    setMsg(null);
    setBusy(true);
    const res = await fetch("/api/payments/marriage-create-invoice", {
      method: "POST",
