...
// JOBS_BOT's own deposit page — a separate component from
// src/app/pay/PayClient.tsx (AD_BOT) and src/app/pay/marriage/MarriagePayClient.tsx
// (MARRIAGE_BOT) so it always posts to /api/payments/jobs-create-invoice.
export default function JobsPayClient({ uid, justPaid }: { uid: string; justPaid: boolean }) {
  ...
  async function pay() {
    setMsg(null);
    setBusy(true);
    const res = await fetch("/api/payments/jobs-create-invoice", {
      method: "POST",
